...
class OpenAIChat(IntelligenceBackend):
    ...
    def _get_response_gen(self, messages, temperature, top_p, max_new_tokens):
        ...
        res = openai.ChatCompletion.create(
            model=DEFAULT_MODEL, messages=messages, temperature=temperature, stream=True
        )
        text = ""
        for chunk in res:
    ...
    def query_with_conclusion(self, agent_name: str, role_desc: str, history_messages: List[Message], global_prompt: str = None,
        ...
        if is_end:
            request_prompt = [{"role": "user", "content": "[主席]：现在结辩，请用一段话总结你自己持方的观点。"}]
        response = self._get_response([system_prompt] + conversations + request_prompt, *args, **kwargs)

        print('-'*80)
        print([system_prompt] + conversations + request_prompt)
        print()
        print(response)
        print('-'*80)
    ...
    def query_with_conclusion_gen(self, agent_name: str, role_desc: str, history_messages: List[Message], global_prompt: str = None,
        ...
        gen = self._get_response_gen([system_prompt] + conversations + request_prompt, *args, **kwargs)
